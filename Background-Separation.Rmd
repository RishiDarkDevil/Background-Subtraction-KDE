...
plot(grayscale(as.cimg(vid[,,100,])))
```
...
prob.kde[prob.kde >= 0.123] <- 1
prob.kde[prob.kde < 0.123] <- 0
plot(as.cimg((1-prob.kde)), main = "Density Values of Frame cutoff at Threshold")
par(mfrow = c(1,1))
```

- We consider another video footage of traffic on a road at the 20th Frame.
```{r traffic_pic_ori, fig.asp=1, fig.width=12, warning=FALSE, cache=TRUE}
vid <- load.video("traffic2.mp4")
plot(grayscale(as.cimg(vid[,,20,])))
```
![Traffic Surveillance](traffic.gif)

```{r run_fun_traffic, fig.asp=1, fig.width=12, warning=FALSE, cache=TRUE}
sep1 <- background.separat.frame.KDE("traffic_gray.mp4", 20, 0.2)
```

```{r background_sep_frame_plot_traffic, fig.asp=1, fig.width=12, warning=FALSE}
par(mfrow = c(2,2))
plot(grayscale(as.cimg(vid[,,20,])), main = "Original Frame")
plot(sep1, main = "Background Separated Frame")
plot(as.cimg((1-prob.kde)), main = "Density Values of Frame")
prob.kde[prob.kde >= 0.2] <- 1
prob.kde[prob.kde < 0.2] <- 0
plot(as.cimg((1-prob.kde)), main = "Density Values of Frame cutoff at Threshold")
par(mfrow = c(1,1))
```